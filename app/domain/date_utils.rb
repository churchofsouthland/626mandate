class DateUtils
  ...
  def self.mandate_start_date
    Time.parse(Figaro.env.mandate_start_date).in_time_zone("Pacific Time (US & Canada)").beginning_of_day
  end

  # mandate start/end dates are in PT
  def self.mandate_finish_date
    Time.parse(Figaro.env.mandate_finish_date).in_time_zone("Pacific Time (US & Canada)").end_of_day
  end
